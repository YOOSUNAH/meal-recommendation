...
public class CsvProcessor {
  ...
  private List<CsvDto> readAllFromCsv() throws IOException {
    ...
    try {
      Path csvFilePath =
          CsvConstants.DESTINATION_DIRECTORY.resolve(
  ...
  public void transCoordinate() {
    ...
    for (Restaurant restaurant : newRestaurant) {
      ProjCoordinate coordinate =
          coordinateConvertor.transformToWGS(restaurant.getLongitude(), restaurant.getLatitude());
  ...
  @Async
  public CompletableFuture<Void> processPageAsync(int page, List<CsvDto> csvDtoList) {
    return CompletableFuture.runAsync(
        () -> {
          log.debug(
              "##### === Async 실행 중 (Thread: {}) page : {}  ",
          ...
          try {
            ...
            List<String> managementNumbers =
                csvDtoList.stream()
                    .map(CsvDto::getManagementNumber) // 람다 표현식 : csvData ->
                    // csvData.getManagementNumber()
                    .collect(Collectors.toList());

            // 2. 해당 managementNumber에 해당하는 Restaurant만 조회
            Map<String, Restaurant> batchMap =
                restaurantRepository.findAllByManagementNumberIn(managementNumbers).stream()
                    .collect(
                        Collectors.toMap(
                            Restaurant::getManagementNumber, restaurant -> restaurant));
            ...
            for (CsvDto csvdata : csvDtoList) {
              ...
              Restaurant restaurant =
                  batchMap.getOrDefault(csvdata.getManagementNumber(), new Restaurant());
  ...
  private void setRestaurantInfo(Restaurant restaurant, CsvDto csvdata) {
    ...
    ProjCoordinate transformed =
        coordinateConvertor.transformToWGS(csvdata.getLongitude(), csvdata.getLatitude());
