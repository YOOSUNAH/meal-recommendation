...
public class OJMService {
  ...
  public List<RestaurantResponseDto> getRandomRestaurants(
      double currentLat, double currentLon, List<String> selectedCategories) {
    List<Restaurant> recommendRestaurants =
        getRecommendRestaurants(currentLat, currentLon, selectedCategories, 300);

    Collections.shuffle(recommendRestaurants);
    List<Restaurant> randomRestaurants = recommendRestaurants.stream().limit(10).toList();

    return randomRestaurants.stream()
        .map(
            r ->
                new RestaurantResponseDto(
                    r.getName(), r.getCategory(), r.getAddress(), r.getNumber()))
        .toList();
  }
  ...
  public List<RestaurantResponseDto> getClosestRestaurants(
      double currentLat, double currentLon, List<String> selectedCategories) {
    List<Restaurant> recommendRestaurants =
        getRecommendRestaurants(currentLat, currentLon, selectedCategories, 200);

    // 거리 순으로 정렬
    recommendRestaurants.sort(
        (r1, r2) -> {
          double distance1 =
              RestaurantDistanceCalculateService.distance(
                  currentLat, currentLon, r1.getLatitude(), r1.getLongitude());
          double distance2 =
              RestaurantDistanceCalculateService.distance(
                  currentLat, currentLon, r2.getLatitude(), r2.getLongitude());
  ...
  public List<Restaurant> getRecommendRestaurants(
      double currentLat, double currentLon, List<String> categories, double maxDistance) {
    ...
    return restaurants.stream()
        .filter(
            restaurant ->
                RestaurantDistanceCalculateService.distance(
                        currentLat, currentLon, restaurant.getLatitude(), restaurant.getLongitude())
                    <= maxDistance)
