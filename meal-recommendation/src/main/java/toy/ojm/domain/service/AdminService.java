...
public class AdminService {
  ...
  public RestaurantPageableResponseDto getAllRestaurants(
      String id, int page, int size, String category, String keyword) {
    validateUser(id);
    Pageable pageable = PageRequest.of(page, size);

    Page<Restaurant> responseDtoPage =
        restaurantQueryRepository.findByCategoryAndKeyword(pageable, category, keyword);
    ...
    return new RestaurantPageableResponseDto(
        responseDtoPage.getContent(), size, page + 1, totalPage, totalElements);
  }

  private Users validateUser(String id) {
    return userRepository
        .findById(id)
        .orElseThrow(() -> new IllegalArgumentException("admin user 가 존재하지 않습니다."));
  }
}
