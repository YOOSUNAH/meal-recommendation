...
public class AdminController {
  ...
  public ResponseEntity<ResponseDto<Void>> login(
      HttpServletRequest request, @RequestBody LoginRequestDto requestDto) {
    // id, pw 일치하는지 확인
  ...
  public void createSession(HttpServletRequest request, @RequestParam String id) {
  ...
  public ResponseEntity<ResponseDto<String>> getSession(HttpServletRequest request) {
    ...
    String value =
        (String)
            session.getAttribute(AUTHORIZATION_HEADER); // 가져온 세션에 저장된 Value 를 Name 을 사용하여 가져옵니다.
  ...
  public ResponseEntity<ResponseDto<RestaurantPageableResponseDto>> getAllRestaurants(
    ...
    RestaurantPageableResponseDto allRestaurants =
        adminService.getAllRestaurants(userId, page, size, category, keyword);
